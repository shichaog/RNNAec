...
Subtractor::Subtractor(const EchoCanceller3Config& config,
  ...
  fp_aec3lp_main = fopen("aec3lp_main.pcm", "wb+");
  fp_aec3lp_shadow = fopen("aec3lp_shadow.pcm", "wb+");
}

//Subtractor::~Subtractor() = default;
Subtractor::~Subtractor() {
	fclose(fp_aec3lp_main);
	fp_aec3lp_main = NULL;	
	fclose(fp_aec3lp_shadow);
	fp_aec3lp_shadow = NULL;
 }
...
void Subtractor::Process(const RenderBuffer& render_buffer,
  ...
  data_dumper_->DumpWav("aec3_shadow_filter_output", kBlockSize, &e_shadow[0],
                        16000, 1);
  int16_t aeclp_main[64];
  int16_t aeclp_shadow[64];
  for (size_t i = 0; i < kBlockSize; i++)
  {
	  aeclp_main[i] = (int16_t)e_main[i];
	  aeclp_shadow[i] = (int16_t)e_shadow[i];
  }
  fwrite(&aeclp_main[0], sizeof(int16_t), 64, fp_aec3lp_main);
  fwrite(&aeclp_shadow[0], sizeof(int16_t), 64, fp_aec3lp_shadow);
}

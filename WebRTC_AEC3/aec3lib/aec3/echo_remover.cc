...
class EchoRemoverImpl final : public EchoRemover {
 ...
 private:
  ...
  //kyyang debug 
  FILE* fp_echoremover_far;
  FILE* fp_echoremover_near;
...
EchoRemoverImpl::EchoRemoverImpl(const EchoCanceller3Config& config,
  ...
  y_old_.fill(0.f);
  e_old_.fill(0.f);
  fp_echoremover_far = fopen("echoremover_far.pcm", "wb+");
  fp_echoremover_near = fopen("echoremover_near.pcm", "wb+");
}

//EchoRemoverImpl::~EchoRemoverImpl() = default;
EchoRemoverImpl::~EchoRemoverImpl() {
	fclose(fp_echoremover_far);
	fp_echoremover_far = NULL;
	fclose(fp_echoremover_near);
	fp_echoremover_near = NULL;
}
...
void EchoRemoverImpl::ProcessCapture(
 ...
 //kyyang debug begin  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  int16_t nearcap[kBlockSize];
  int16_t render[kBlockSize];
  for (size_t i = 0; i < kBlockSize; i++) {

	  if (x0[i] > 32767) render[i] = 32767;
	  else if (x0[i] < -32768) render[i] = -32768;
	  else render[i] = (int16_t)x0[i];

	  if (y0[i] > 32767) nearcap[i] = 32767;
	  else if (y0[i] < -32768) nearcap[i] = -32768;
	  else nearcap[i] = (int16_t)y0[i];

  }
  fwrite(nearcap, sizeof(int16_t), 64, fp_echoremover_near);
  fwrite(render, sizeof(int16_t), 64, fp_echoremover_far);
//kyyang debug <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
